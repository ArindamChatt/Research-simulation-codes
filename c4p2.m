...
NT = 256;                          % Max users in the system (fixed)
max_split_ratio = 256;

% XG-PON or similar parameters changed to TWDM
Pport = 20;                        % Power per OLT port (W)
DS = 5e9;                          % Downstream rate (bps)
US = 5e9;                          % Upstream rate (bps) TWDM
...
title('P_{CO} vs B_m at 10% Availability Violation (256 Users, Line Rate = 50 Gbps)');

figure; grid on; hold on;
plot(Bm_values / 1e6, S_values, 'b-o', 'LineWidth', 2);
xlabel('Average Offered Bandwidth per User, B_m (Mbps)');
ylabel('Split ratio (S)');
title('S vs B_m at 10% Availability Violation (256 Users, Line Rate = 50 Gbps)');
